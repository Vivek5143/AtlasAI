...
const PROBLEM_LABELS: Partial<Record<keyof Problem, string>> = {
  external_problem_id: "External Problem ID",
  problem_type: "Problem Type",
  vc_stage: "VC Stage",
  financial_impact: "Financial Impact",
  regulatory_trigger: "Regulatory Trigger",
  ai_solution: "AI Solution",
  created_at: "Created At",
  updated_at: "Updated At",
};

function toFieldLabel(field: keyof Problem): string {
  const customLabel = PROBLEM_LABELS[field];
  if (customLabel) {
    return customLabel;
  }
...
function formatProblemFieldValue(
  field: keyof Problem,
  value: Problem[keyof Problem],
): string {
  if (value === null || value === "") {
    return "—";
  }
...
function ProblemSkeletonCard(): ReactElement {
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-4 h-3 w-full animate-pulse rounded bg-slate-200 dark:bg-slate-800" />
...
function ProblemDetailsDrawer({
  ...
  const entries = Object.entries(problem) as [
    keyof Problem,
    Problem[keyof Problem],
  ][];
          ...
          {entries.map(([field, value]) => (
            ...
            >
              ...
              <p className="mt-2 break-words text-sm text-slate-800 dark:text-slate-200">
                {formatProblemFieldValue(field, value)}
              </p>
            </article>
          ))}
        </div>
...
export function ProblemsPage(): ReactElement {
  ...
  return (
    <PageContainer
      title="Problems"
      subtitle="Analyze persisted business problems and their operational metadata."
    >
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200">
              Problem Count: {totalProblems.toLocaleString()}
            </span>
            <button
              type="button"
              onClick={() => {
        ...
        {!isLoading && isError ? (
          <section className="rounded-2xl border border-red-200 bg-red-50 p-5 dark:border-red-900/60 dark:bg-red-950/30">
            ...
            <p className="mt-2 text-sm text-red-600 dark:text-red-200/80">
              We could not fetch problems from the backend. Please retry.
            </p>
            <button
              type="button"
              onClick={() => {
        ...
        {!isLoading && !isError && problems.length === 0 ? (
          <section className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-center dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-base font-semibold text-slate-950 dark:text-white">
              {searchTerm.trim() ? "No matching problems" : "No problems available"}
            </h2>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              {searchTerm.trim()
                ? "Try a broader keyword."
                : "The backend returned an empty problems dataset."}
            </p>
          </section>
        ) : null}
              ...
              >
                ...
                <p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-500 dark:text-slate-400">
                  {problem.financial_impact ||
                    ...
                    problem.ai_solution ||
                    "No additional summary is available for this problem yet."}
                </p>
              </article>
            ))}
          </div>
